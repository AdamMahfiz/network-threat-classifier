...
def check_session_timeout():
    ...
    # Skip timeout check for auth routes, static files, and API routes
    if (request.endpoint and 
        (request.endpoint.startswith('auth.') or 
         request.endpoint.startswith('static') or
         request.path.startswith('/api/') or
         request.path.startswith('/socket.io') or
         request.endpoint in ['unauthorized'])):
        return
...
def send_email_alert(subject, body, high_count=None, timestamp=None):
    try:
        with app.app_context():
            ...
            if high_count is not None and timestamp is not None:
                ...
                body = (
                    ...
                    f"This is an automated alert from the official Network Threat Classification System.\n\n"
                    f"A total of {high_count} high-level threats were detected during the latest analysis.\n"
                    f"Timestamp: {timestamp}\n\n"
                    f"Please review the dashboard for further details and take appropriate action if necessary.\n\n"
                    f"Best regards,\n"
                    f"Network Threat Classification System (Automated Machine)"
                )
                ...
                if len(df.columns) >= 41:
                    ...
                    if nsl_kdd_processor:
                        ...
                        if high_count > 0:
                            ...
                            send_email_alert('High Threat Alert', f'{high_count} high threats detected!')
                            socketio.emit('live_alert', alert)
...
@login_required
def audit_page():
    # Only admins can access audit page
